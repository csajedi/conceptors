...
#     return np.vstack((x, patterns))
  ...
  def train_pattern(self,
    ...
    q_collector=np.zeros((self.num_out, self.learn_length));         # 矩阵Q
    ...
    for n in xrange(self.washout_length+self.learn_length):
      ...
      v = result[:, n][None].T;     # v = 第n时间的 result
      x_old=x;                      # x_old
      ...
      if n>self.washout_length-1:
        ...
        q_collector[:, n - self.washout_length] = v[:, 0];  # P = [...]
    ...
    if not self.all_train_outs.size:
      self.all_train_outs=q_collector
    else:
      self.all_train_outs=np.hstack((self.all_train_outs, q_collector))
  ...
  def train(self,
    ...
    for i in xrange(len(patterns)):
      self.train_pattern(patterns[i], results[i])
      # train all patterns
